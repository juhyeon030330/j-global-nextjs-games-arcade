                  ...
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                    <div>
                      ...
                      <textarea
                        ...
                        maxLength={500}
                        rows={3}
                        placeholder="Enter game overview (English)"
                        className="w-full px-3 py-2 border rounded-xl text-sm bg-white resize-none"
                      />
                      <div className="text-right text-[10px] text-slate-400 mt-0.5">
                        {(editingGame.description || "").length} / 500
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1">
                        Game Overview (Japanese)
                      </label>
                      <textarea
                        value={editingGame.description_ja || ""}
                        onChange={(e) =>
                          setEditingGame({
                            ...editingGame,
                            description_ja: e.target.value.slice(0, 500),
                          })
                        }
                        maxLength={500}
                        rows={3}
                        placeholder="Enter game overview (Japanese)"
                        className="w-full px-3 py-2 border rounded-xl text-sm bg-white resize-none"
                      />
                      <div className="text-right text-[10px] text-slate-400 mt-0.5">
                        {(editingGame.description_ja || "").length} / 500
                      </div>
                    </div>
                  </div>
